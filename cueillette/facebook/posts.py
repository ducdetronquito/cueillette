...
class _Post:
    ...
    def get(
        ...
        page_id = cls._get_page_id(profile)
        posts = _Post(
            profile=profile,
        ...
        return [
            ...
            for html_post in html_page.find_class('userContentWrapper')
        ]
    ...
    def _extract_metadata(self, html_post) -> dict:
        ...
        """
        metadata = {}
        # The publication timestamp of a post can be found in
        # the publication date block located in this post header.
        timestamp_block = html_post.find_class('_5ptz')[0]
        publication_utc_timestamp = timestamp_block.get('data-utime')
        if publication_utc_timestamp:
            metadata['publication_utc_timestamp'] = publication_utc_timestamp

        # The publication date located in a post header contains
        # a link to this post. 
        post_url = timestamp_block.getparent().get('href')
        if post_url:
            metadata['url'] = 'https://facebook.com' + post_url
        
        # The name of the post's author is located inside a h5 block.
        author_block = html_post.find('.//h5')
        author_profile_link_tag = author_block.find('.//a')
        if author_profile_link_tag is not None:
            metadata['author'] = author_profile_link_tag.text
